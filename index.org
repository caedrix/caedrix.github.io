...
#+BEGIN_EXAMPLE shell
*> stabs = [cycle(k, X[0, 3]*Z[1, 2]) for k in range(4)]
*> P = prod((I + S)/2 for S in stabs)
*> errs = [E[k] for k in range(5) for E in (X, Y, Z)]
*> def kl(A, B): return scalar(P * A.d * B * P, P)
*> all(kl(A, B) is not None for A in errs for B in errs)
True
#+END_EXAMPLE
...
Compile a ~.yaw~ file:
#+BEGIN_EXAMPLE shell
python yaw/y2py.py program.yaw -o program.py
python program.py
#+END_EXAMPLE
Import as package:
...
#+END_EXAMPLE
* SIQP II
This site hosts /Structure and Interpretation of Quantum Programs II:
The Way of Yaw/, a living textbook which develops the language from scratch and uses it
to build a canonical set of quantum algorithms. Here is the live table
of contents:
...
|-----+----------------------+----------|
| 1.1 | Algebras             | live     |
| 1.2 | Operators and States | live     |
| 1.3 | Laws of composition  | live     |
|-----+----------------------+----------|
|     | ALGORITHMS           |          |
|-----+----------------------+----------|
| 2.1 | Send                 | live     |
| 2.2 | Search               | live     |
| 2.3 | Sample               | live     |
| 2.4 | Simulate             | drafting |   
| 2.5 | Subgroup             | planned  |
| 2.6 | Shadow               | planned  |
| 2.7 | Signal               | planned  |
| 2.8 | Sparsity             | planned  |
|-----+----------------------+----------|
|     | APPENDICES           |          |
|-----+----------------------+----------|
|   A | Exercises            | live     |
|   B | Solutions            | drafting |
|   C | Awds                 | planned  |
|   D | Reference programs   | planned  |
* FAQs
Q. /Isn't this just a library with fancy syntax?/ \\
A. No, it's a different way of thinking about quantum computing and an
accompanying language to implement that thinking.
...
A. Soon! This is a neat generalization of the algebra of qudits.
...
and the tools already exist for it. Implementation is a work in progress.
...
A. You can get by with high school algebra!

Q. /What does "yaw" mean?/ \\
A. It is a self-conjugating recursive acronym for *Yaw Algebraic Way*.

Q. /Where are the solutions to exercise?/ \\
...
Foundations/. Torsor Labs, 2025. [[https://arxiv.org/abs/2509.04527][arXiv:2509.04527]].
