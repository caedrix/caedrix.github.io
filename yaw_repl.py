...
class YawREPL:
    ...
    def eval_line(self, line):
        ...
        # Match "name = ..." where the = is a real assignment, not part of
        # ==, <=, >= or !=. Testing for those operators anywhere in the line
        # is wrong: the right-hand side may legitimately contain a comparison,
        # as in  f = lambda a, b: 1 if a % 2 == 0 else 0
        if re.match(r'^[A-Za-z_]\w*(\s*,\s*[A-Za-z_]\w*)*\s*=(?!=)', line_stripped):
